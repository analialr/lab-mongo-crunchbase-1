        ...
        switch (option) {
          ...
          case "3":
            ...
            case "5":
             ...
             db.collection('companies').find({$or: [{founded_year: 2004, founded_month: 4},{founded_year: 2004, founded_month: 5},{founded_year: 2004, founded_month: 6}]}, { name: 1, _id: 0 }).sort(founded_month).toArray((error, result) => {
               if (error) {
                 console.log(error);
                 rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
               } else {
             ...
             // https://docs.mongodb.com/manual/tutorial/project-fields-from-query-results/#project-fields-to-return-from-query
             db.collection('companies').find({name: "Facebook"}).project({ "products.name": 1 }).toArray((error, result) => {
               if (error) {
                 console.log(error);
                 rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
               } else {
